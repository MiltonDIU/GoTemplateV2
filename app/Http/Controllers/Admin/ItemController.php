	   ...
	   else
	   {
		...
		else
		{
		 ...
		 $updated_item = date('Y-m-d H:i:s');
		    ...
		    $data = array('item_name' => $item_name, 'item_desc' => $item_desc, 'item_thumbnail' => $item_thumbnail, 'item_preview' => $item_preview, 'item_file' => $item_file, 'item_category' =>$cat_id, 'item_category_type' => $cat_name, 'item_type' => $item_type, 'regular_price' => $regular_price,'regular_licence' => $regular_licence, 'extended_price' => $extended_price, 'extended_licence' => $extended_licence, 'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'free_download' => $free_download, 'item_slug' => $item_slug, 'video_url' => $video_url, 'future_update' => $future_update, 'item_support' => $item_support, 'updated_item' => $updated_item, 'item_flash' => $item_flash, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file);
...
//    return view('admin.ssl_transaction');
    ...
    public function viewExclusiveVendorRequest() {
        $users = User::where('exclusive_author_need_approval',1)->where('exclusive_author',0)->get();
        return view('admin.exclusive_author_need_approval',compact('users'));
    }

public function viewExclusiveVendorRequestAppoved($id){
   $user = User::find($id);
   $data['exclusive_author_need_approval']=1;
   $data['exclusive_author']=1;
   $user->update($data);
    return redirect()->back()->with('success','Exclusive Vendor request has been approved completed');
}
}
