      ...
      if (videoPreviewType && videoPreviewType != '') {
        if (videoPreviewType == 'youtube') {
          if (videoUrl && videoUrl != '') {
            ...
            const srcUrl = `{{ url('/') }}/resources/views/assets/no-video.png`;
          ...
          if (videoFile && videoFile != '') {
            ...
            if (<?php echo $allsettings->site_s3_storage ?> == 1) {
              ...
              const srcUrl = `{{ url('/') }}/public/storage/items/${videoFile}`;
              finalElm = `
                                    <video width="100%" height="230" controls loop>
        ...
        if (itemPreview != '') {
          const srcUrl = `{{ url('/') }}/public/storage/items/${itemPreview}`;
          finalElm = `<img src="${srcUrl}" alt="Item Name">`;
        } else {
          const srcUrl = `{{ url('/') }}/public/img/no-image.png`;
          finalElm = `<img src="${srcUrl}" alt="Item Name">`;
        }
      }
